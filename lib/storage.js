...
편집 시 입력한 ''작성자'' 이름은 [[편집 내역]], 역사, 기여 목록 등에서 자동으로 자신의 사용자 문서로 연결됩니다.
...
const USER_NS_PREFIX = '사용자:';

function movePage(oldTitle, newTitle, author, ip, opts) {
  oldTitle = String(oldTitle || '').trim();
  newTitle = String(newTitle || '').trim();
  if (!oldTitle || !newTitle) throw new Error('제목이 비어 있습니다.');
  if (oldTitle === newTitle) throw new Error('원래 제목과 새 제목이 같습니다.');
  if (!db.pages[oldTitle]) throw new Error('원본 문서가 존재하지 않습니다: ' + oldTitle);
  if (db.pages[newTitle]) throw new Error('대상 문서가 이미 존재합니다: ' + newTitle);
  if (db.blocked && db.blocked[newTitle]) throw new Error('대상 제목은 차단된 상태입니다.');

  const revs = db.revisions[oldTitle] || [];
  const last = revs[revs.length - 1];
  // Append a "move" revision so the history records the rename.
  const moveRev = {
    rev: (last ? last.rev : 0) + 1,
    content: last ? last.content : '',
    author: author || 'Anonymous',
    comment: `문서 이동: ${oldTitle} → ${newTitle}`,
    createdAt: new Date().toISOString(),
    ip: ip || '',
    size: last ? last.size : 0,
    delta: 0,
  };
  revs.push(moveRev);

  db.revisions[newTitle] = revs;
  delete db.revisions[oldTitle];

  if (db.discussions[oldTitle]) {
    db.discussions[newTitle] = db.discussions[oldTitle];
    delete db.discussions[oldTitle];
  }

  db.pages[newTitle] = {
    title: newTitle,
    currentRev: moveRev.rev,
    updatedAt: moveRev.createdAt,
  };
  delete db.pages[oldTitle];

  // If this is a 사용자: namespace move, rewrite author strings across the
  // whole wiki so "편집 내역" and contribution lookups reflect the new name.
  let renamedAuthor = null;
  if (oldTitle.startsWith(USER_NS_PREFIX) && newTitle.startsWith(USER_NS_PREFIX)) {
    const oldName = oldTitle.slice(USER_NS_PREFIX.length);
    const newName = newTitle.slice(USER_NS_PREFIX.length);
    if (oldName && newName && oldName !== newName) {
      Object.values(db.revisions).forEach((arr) => {
        arr.forEach((r) => { if (r.author === oldName) r.author = newName; });
      });
      Object.values(db.discussions).forEach((arr) => {
        arr.forEach((c) => { if (c.author === oldName) c.author = newName; });
      });
      // Update the move revision's author too if it was made by the user themselves
      if (moveRev.author === oldName) moveRev.author = newName;
      renamedAuthor = { oldName, newName };
    }
  }

  save();
  // Notify caller (server.js) so it can update the user account's displayName.
  if (renamedAuthor && opts && typeof opts.onUserRename === 'function') {
    opts.onUserRename(renamedAuthor.oldName, renamedAuthor.newName);
  }
  return moveRev;
}

function blockPage(title, info) {
  if (!db.blocked) db.blocked = {};
  db.blocked[title] = {
    reason: (info && info.reason) || '관리자에 의해 접근이 차단되었습니다.',
    movedTo: (info && info.movedTo) || null,
    by: (info && info.by) || null,
    at: new Date().toISOString(),
  };
  save();
}

function unblockPage(title) {
  if (db.blocked) {
    delete db.blocked[title];
    save();
  }
}

function isBlocked(title) {
  return !!(db.blocked && db.blocked[title]);
}

function getBlockInfo(title) {
  return (db.blocked && db.blocked[title]) || null;
}

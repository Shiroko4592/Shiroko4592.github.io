...
// Resolve a video shortcode/URL into an embed src URL.
// Returns { provider, src } or null if it cannot be resolved.
function resolveVideoEmbed(kind, value) {
  const k = String(kind || '').toLowerCase();
  const v = String(value || '').trim();
  if (!v) return null;

  // Generic auto-detection from a full URL
  if (k === '동영상' || k === 'video') {
    if (/(?:youtube\.com|youtu\.be)/i.test(v)) return resolveVideoEmbed('youtube', v);
    if (/nicovideo\.jp/i.test(v)) return resolveVideoEmbed('nicovideo', v);
    if (/bilibili\.com/i.test(v)) return resolveVideoEmbed('bilibili', v);
    if (/tiktok\.com/i.test(v)) return resolveVideoEmbed('tiktok', v);
    if (/vimeo\.com/i.test(v)) return resolveVideoEmbed('vimeo', v);
    return null;
  }
  ...
  if (k === 'youtube' || k === '유튜브') {
    let id = v;
    let m;
    if ((m = v.match(/(?:youtu\.be\/|[?&]v=|\/embed\/|\/shorts\/|\/v\/)([\w-]{6,})/))) id = m[1];
    if (!/^[\w-]{6,}$/.test(id)) return null;
    return { provider: 'youtube', src: `https://www.youtube.com/embed/${id}` };
  }

  if (k === 'nico' || k === 'nicovideo' || k === '니코' || k === '니코니코') {
    let id = v;
    let m;
    if ((m = v.match(/watch\/((?:sm|nm|so)?\w+)/i))) id = m[1];
    if (!/^[\w]+$/.test(id)) return null;
    return { provider: 'nicovideo', src: `https://embed.nicovideo.jp/watch/${id}` };
  }

  if (k === 'bilibili' || k === '빌리빌리') {
    let m;
    if ((m = v.match(/\/video\/(BV[\w]+)/i))) return { provider: 'bilibili', src: `https://player.bilibili.com/player.html?bvid=${m[1]}&high_quality=1` };
    if ((m = v.match(/\/video\/av(\d+)/i))) return { provider: 'bilibili', src: `https://player.bilibili.com/player.html?aid=${m[1]}&high_quality=1` };
    if (/^BV[\w]+$/i.test(v)) return { provider: 'bilibili', src: `https://player.bilibili.com/player.html?bvid=${v}&high_quality=1` };
    if (/^av\d+$/i.test(v)) return { provider: 'bilibili', src: `https://player.bilibili.com/player.html?aid=${v.slice(2)}&high_quality=1` };
    if (/^\d+$/.test(v)) return { provider: 'bilibili', src: `https://player.bilibili.com/player.html?aid=${v}&high_quality=1` };
    return null;
  }
  ...
  if (k === 'tiktok' || k === '틱톡') {
    let id = v;
    let m;
    if ((m = v.match(/\/video\/(\d+)/))) id = m[1];
    if (!/^\d+$/.test(id)) return null;
    return { provider: 'tiktok', src: `https://www.tiktok.com/embed/v2/${id}` };
  }

  if (k === 'vimeo' || k === '비메오') {
    let id = v;
    let m;
    if ((m = v.match(/vimeo\.com\/(?:video\/|channels\/[^/]+\/|groups\/[^/]+\/videos\/)?(\d+)/))) id = m[1];
    if (!/^\d+$/.test(id)) return null;
...
function parseInline(text, ctx) {
  ...
  // Stored as a placeholder so the <sup> tag survives the later escapeHtml() pass.
  s = s.replace(/\[\*([^\s\]]*)\s*([^\]]+)\]/g, (_, name, content) => {
    const idx = ctx.footnotes.length + 1;
    ctx.footnotes.push(parseInline(content.trim(), ctx));
    placeholders.push(`<sup class="fn"><a id="fnref-${idx}" href="#fn-${idx}">[${idx}]</a></sup>`);
    return `\x00P${placeholders.length - 1}\x00`;
  });

  // Video embed: [[유튜브:ID|opts]] / [[youtube:URL]] / [[니코니코:sm9]] / [[빌리빌리:BV...]] / [[틱톡:videoId]] / [[비메오:videoId]] / [[동영상:URL]]
  s = s.replace(/\[\[(동영상|video|youtube|유튜브|nico|nicovideo|니코|니코니코|bilibili|빌리빌리|tiktok|틱톡|vimeo|비메오):([^\]|]+)(?:\|([^\]]+))?\]\]/gi, (m, kind, idOrUrl, opts) => {
    const info = resolveVideoEmbed(kind, idOrUrl.trim());
    if (!info) return m; // leave unchanged so user can correct it
    ...
    if (opts) {
      ...
      for (const p of parts) {
        const kv = p.split('=');
        if (kv.length === 2) {
          const kk = kv[0].trim().toLowerCase();
          const vv = kv[1].trim();
          if (kk === 'width') width = /^\d+$/.test(vv) ? vv + 'px' : vv;
          else if (kk === 'height') height = /^\d+$/.test(vv) ? vv + 'px' : vv;
          else if (kk === 'align') align = vv.toLowerCase();
        }
      }
    }
    let wrapStyle = '';
    if (width) wrapStyle += `max-width:${width};`;
    ...
    else if (align === 'left') wrapStyle += 'margin-left:0;margin-right:auto;';
    const frameStyle = height ? `padding-bottom:0;height:${height};` : '';
    placeholders.push(
      `<span class="wvideo wvideo-${info.provider}"${wrapStyle ? ` style="${wrapStyle}"` : ''}>` +
      `<span class="wvideo-frame"${frameStyle ? ` style="${frameStyle}"` : ''}>` +
      `<iframe src="${escapeHtml(info.src)}" frameborder="0" ` +

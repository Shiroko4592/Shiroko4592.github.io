...
// Returns the user's display name (defaults to phone for accounts with no
// custom display name). All views/topnav/recent-changes use this.
function getDisplayName(user) {
  if (!user) return '';
  return user.displayName || user.phone;
}

// Rename the display name of any account whose current display name matches
// `oldName`. Used when a 사용자: page is moved so the topnav and any future
// editor "author" defaults reflect the new name. Returns true if anything
// was updated.
function renameDisplayName(oldName, newName) {
  let updated = false;
  for (const phone in db.users) {
    const u = db.users[phone];
    const current = u.displayName || u.phone;
    if (current === oldName) {
